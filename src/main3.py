...
def validate(model: Model, loader: DataLoaderIAM, line_mode: bool) -> Tuple[float, float]:
    ...
    while loader.has_next():
        ...
        for i in range(len(recognized)):
            ...
            num_word_ok += 1 if gt == pred else 0
            num_word_total += 1
            dist = editdistance.eval(pred, gt)
            num_char_err += dist
            num_char_total += len(gt)
            print('[OK]' if dist == 0 else '[ERR:%d]' % dist, '"' + gt + '"', '->', '"' + pred + '"')
    ...
    return char_error_rate, word_accuracy, f1, cm
...
def main():
   ...
   cm = validate(model, loader, line_mode=args.line_mode)
plot_confusion_matrix(cm, labels=loader.char_list)
