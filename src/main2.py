...
def validate(model: Model, loader: DataLoaderIAM, line_mode: bool) -> Tuple[float, float]:
    """Validates NN and computes metrics."""
    print('Validate NN')
    ...
    while loader.has_next():
        ...
        all_gt_texts.extend(batch.gt_texts)
        all_recognized_texts.extend(recognized)
    ...
    # Flatten GT and recognized texts for metrics calculation
    flattened_gt = ''.join(all_gt_texts)
    flattened_recognized = ''.join(all_recognized_texts)
    unique_chars = sorted(set(flattened_gt))  # Unique characters in GT texts

    # Convert texts to label indices
    label_mapping = {ch: i for i, ch in enumerate(unique_chars)}
    gt_labels = [label_mapping[ch] for ch in flattened_gt]
    recognized_labels = [label_mapping.get(ch, -1) for ch in flattened_recognized if ch in label_mapping]

    # Ensure same lengths
    recognized_labels = recognized_labels[:len(gt_labels)]

    # Compute metrics
    cm = confusion_matrix(gt_labels, recognized_labels, labels=range(len(unique_chars)))
    f1 = f1_score(gt_labels, recognized_labels, average='weighted')
